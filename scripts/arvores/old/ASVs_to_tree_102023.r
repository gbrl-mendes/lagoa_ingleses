...
fish_ID_tbl_fasta <- grouped_by_ID_BLASTid %>%
    # filter(`Primer expected length` == "in range") %>% 11/23 para obter a arvore de ASVs e definir as IDs
  # filter(RRA >= 0.01) %>%
  # filter(!(`ASV header` %in% c(">ASV_384_175bp", #ASVs de Teleo
  #                          ">ASV_15_167bp",
  #                          ">ASV_326_175bp", 
  #                          ">ASV_201_172bp",
  #                          ">ASV_146_170bp"))) %>%  
  mutate(`Curated ID` = gsub(" ", "_", `Curated ID`)) %>% # Unindo os nomes com _
  unite(header, `ASV header`, `Curated ID`, sep = "|") %>%  # Criando uma nova coluna com o cabeçalho da ASV e o nome de spp atribuido # em 11/23 para obter a arvore de ASVs e definir as IDs
...
# write(ASVs_001, "/home/gabriel/projetos/lagoa_ingleses/results/tree/2023/1023_ASVs_001_filt.fas") # Criando o arquivo

ASVs_nofilt <- c(rbind(fish_ID_tbl_fasta$header, fish_ID_tbl_fasta$`ASV (Sequence)`)) # Unindo as informacoes em formato FASTA
write(ASVs_nofilt, "/home/gabriel/projetos/lagoa_ingleses/results/tree/2023/1023_ASVs_nofilt.fas") # Criando o arquivo


  # Criando objeto Biostring para fazer alinhamento 
# algn_in_ASVs_001 <- readDNAStringSet(filepath = "/home/gabriel/projetos/lagoa_ingleses/results/tree/2023/1023_ASVs_001_filt.fas")
# BrowseSeqs(algn_in_ASVs_001)

algn_in_ASVs_nofilt <- readDNAStringSet(filepath = "/home/gabriel/projetos/lagoa_ingleses/results/tree/2023/1023_ASVs_nofilt.fas")
...
ape::write.tree(phy = dna_tree, file = "~/projetos/lagoa_ingleses/results/tree/2023/asv-LI_db_tree_nofilt_clean.nwk")
