...
fish_ID_tbl_fasta <- grouped_by_ID_BLASTid %>%
  ...
  # filter(`Final ID (BLASTn)`%in% c("Astyanax lacustris", "Astyanax paranae", "Astyanax bimaculatus")) %>% #### corrigir
  ...
  # filter(`Final ID (BLASTn)`%in% c("Pseudoplatystoma corruscans", "Iheringichthys labrosus", "Pimelodus fur", "Pimelodus mysteriosus", "Pimelodus albicans", "Pimelodus maculatus")) %>%
...
## Criando o FASTA ----
{
  ...
  write(ASVs_all, "/home/gabriel/projetos/lagoa_ingleses/results/tree/2023/distancia_grupos_arvore/ASVs.fas") # Criando o arquivo  
}


# Lendo o FASTA
algn_in_ASVs_nofilt <- readDNAStringSet(filepath = "/home/gabriel/projetos/lagoa_ingleses/results/tree/2023/distancia_grupos_arvore/ASVs.fas")
